#include "servers/ee_move_action_server.h"

template <typename ActionT>
EEMoveActionServer<ActionT>::EEMoveActionServer(const std::string &name, const rclcpp::Node::SharedPtr &node): Base(name, node)      
{
    this->init();
    RCLCPP_INFO(this->node_->get_logger(), "[%s] EEMoveActionServer constructed", name.c_str());

}

template <typename ActionT>
rclcpp_action::GoalResponse EEMoveActionServer<ActionT>::handleGoal(const rclcpp_action::GoalUUID & /*uuid*/, std::shared_ptr<const Goal> goal)
{
    // Validate input
    ...
    if (len_arms <= 0)
    {
        RCLCPP_WARN(this->node_->get_logger(), "[EEMoveActionServer] Empty arm_names. Rejecting.");
        return rclcpp_action::GoalResponse::REJECT;
    }

    if (goal->execution_time <= 0.0)
    {
        RCLCPP_WARN(this->node_->get_logger(), "[EEMoveActionServer] execution_time <= 0. Rejecting.");
        return rclcpp_action::GoalResponse::REJECT;
    }

    if (this->control_running_.load())
    {
        RCLCPP_WARN(this->node_->get_logger(), "[EEMoveActionServer] Control is already running. Rejecting new goal.");
        return rclcpp_action::GoalResponse::REJECT;
    }
...
template <typename ActionT>
rclcpp_action::CancelResponse EEMoveActionServer<ActionT>::handleCancel(const std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    RCLCPP_INFO(this->node_->get_logger(), "[EEMoveActionServer] Cancel requested.");
    return rclcpp_action::CancelResponse::ACCEPT;
}

template <typename ActionT>
void EEMoveActionServer<ActionT>::handleAccepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    ...
    for (int arm_idx = 0; arm_idx < len_arms; arm_idx++)
    {
        ...
        auto & target_pose = goal_->target_poses[arm_idx];

        active_arms_[arm_name] = true;


        tf::poseMsgToEigen(target_pose, x_ee_d_[arm_name]);
    ...
    RCLCPP_INFO(this->node_->get_logger(), "[EEMoveActionServer] Joint move goal accepted.");

}

template <typename ActionT>
bool EEMoveActionServer<ActionT>::compute(DualHdrUpdater &model)
{
    ...
    if (run_time > duration_)
    {
        ...
        RCLCPP_INFO(this->node_->get_logger(), "[EEMoveActionServer] Joint move goal completed.");
    }
...
template <typename ActionT>
bool EEMoveActionServer<ActionT>::timerReset()
{
...
template class EEMoveActionServer<snu_hdr_msgs::action::JointMove>;