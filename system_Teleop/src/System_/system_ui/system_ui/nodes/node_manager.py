...
class NodeManager(QObject):
    ...
    def handle_calibration(self):
        ...
        if self.switch_side == 'left':
            # self.pm.start(self.process_list[""])
            self.node_.request_left(2,0)      # vision LeftSystemRecorddetect and calibration
            # self.node_.request_left(0,1)
            # self.recv_data = {"title":"Left Record","button_name":"Record", "text":"/Record"}
            # self.excute_command("Left")
