...
class ManusBimanualRetarget(Node):
    def __init__(self):
        ...
        self.left_sub = self.create_subscription(
            JointState, '/dg5f_left/joint_states', self.rc_gripper, 10
        )

        # 퍼블리셔들
        self.left_pub = self.create_publisher(
            MultiDOFCommand, self.left_output_topic, 1
        )
    ...
    def rc_gripper(self, msg: JointState):
        # self.get_logger().info(f"Recv {msg.position}")
        a=1
    ...
    def _compute_mqd_from_q(q_deg, hand_side='left'):
        ...
        qd[5] = q_deg[5] * (PI / 180)
        qd[6] = (q_deg[6]  )* (PI / 180)
        # qd[7] = (q_deg[7] - 10) * (PI / 180)
        qd[7] = q_deg[7] * (PI / 180)
        
        # qd[8] = (q_deg[8]) * (PI / 180)
        qd[8] = (-22) * (PI / 180)
        qd[9] = q_deg[9] * (PI / 180)
        qd[10] = (q_deg[10]) * (PI / 180)
        qd[11] = q_deg[11] * (PI / 180)
        # qd[11] = (q_deg[11]-10) * (PI / 180)
